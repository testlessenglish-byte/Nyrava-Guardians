...
import { HistoryValleyScene } from "@/components/history/history-valley-scene";
import { HistoryValleyMapModal } from "@/components/history/history-valley-map-modal";
...
export const Route = createFileRoute("/world/history-valley")({
  ssr: false,
  component: HistoryValleyPage,
});

function HistoryValleyPage() {
  const quality = QUALITY[useQuality()];
  const active = useAppActive();
  const dragging = useRef(false);
  const navigate = useNavigate();
  const { guardianName } = useGuardian();
  const guardian = CLASS_GUARDIANS.find((g) => g.id === "nova") ?? CLASS_GUARDIANS[0]!;
  const playerColor = guardian.color ?? "#f59e0b";
  const playerLabel = `${guardianName || "Explorer"} · Nova`;
  ...
  const handleLaunchActivity = (activityKey: string) => {
    ...
    completeWorldActivity({ data: { worldId: "history-valley", activityKey } })
  ...
  return (
    <div className="game-viewport history-viewport fixed inset-0 bg-background">
      <div
      ...
      <div className="pointer-events-none absolute left-0 right-0 top-0 flex items-center justify-between p-4">
        <div className="pointer-events-auto flex items-center gap-3">
          <Link
            to="/world"
            className="flex items-center gap-2 rounded-2xl border border-amber-500/30 bg-slate-950/80 px-4 py-2 text-xs font-black text-amber-300 shadow-xl backdrop-blur transition hover:bg-slate-900"
          >
            <ArrowLeft className="h-4 w-4" />
            World Map
          </Link>
          <div className="hidden rounded-2xl border border-amber-500/20 bg-slate-950/80 px-4 py-2 text-xs font-black text-amber-200 backdrop-blur sm:flex">
            🏛️ History Valley
          </div>
        </div>
        ...
        <div className="pointer-events-auto flex items-center gap-3">
          <Button
            ...
            variant="outline"
            className="gap-2 rounded-2xl border-amber-500/30 bg-slate-950/80 text-xs font-black text-amber-300 hover:bg-slate-900"
          >
      ...
      {notification && (
        <div className="pointer-events-none absolute bottom-24 left-1/2 -translate-x-1/2 rounded-2xl border border-amber-400/50 bg-slate-950/90 px-5 py-2.5 text-xs font-black text-amber-300 shadow-2xl backdrop-blur">
          {notification}
        </div>
      )}
